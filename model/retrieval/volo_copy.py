...
class VOLO(nn.Module):
    ...
    def reset_classifier(self, num_classes):
        ...
        if self.return_dense:
            self.aux_head = nn.Linear(
                self.embed_dim,
                num_classes) if num_classes > 0 else nn.Identity()
            self.aux_head.apply(self._init_weights)
    ...
    def forward(self, x):
        ...
        # TODO just return features
        return x.unsqueeze(-1).permute(0, 2, 1, 3)
...
def volo_d2(pretrained=False, **kwargs):
    ...
    if pretrained:
        model.load_state_dict(torch.load(
            "/media/gdut502/139e9283-5fa3-498b-ba3d-0272a299eeeb/wqr/xinye/volo_pretrained/d2_224_85.2.pth.tar"))
    return model
...
def volo_d3(pretrained=False, **kwargs):
    ...
    if pretrained:
        model.load_state_dict(torch.load(
            "/media/gdut502/139e9283-5fa3-498b-ba3d-0272a299eeeb/wqr/xinye/volo_pretrained/d3_224_85.4.pth.tar"))
    return model
...
def volo_d4(pretrained=False, **kwargs):
    ...
    if pretrained:
        model.load_state_dict(torch.load("/media/gdut502/139e9283-5fa3-498b-ba3d-0272a299eeeb/wqr/xinye/volo_pretrained/d4_224_85.7.pth.tar"))
    return model
...
def volo_d5(pretrained=False, **kwargs):
    ...
    model.default_cfg = default_cfgs['volo_large']
    if pretrained:
        model.load_state_dict(torch.load("/media/gdut502/139e9283-5fa3-498b-ba3d-0272a299eeeb/wqr/xinye/volo_pretrained/d5_224_86.10.pth.tar"))
    return model
...
if __name__ == "__main__":
    ...
    model = volo_d1()
    # model.load_state_dict(torch.load("/media/gdut502/139e9283-5fa3-498b-ba3d-0272a299eeeb/wqr/xinye/volo_pretrained/d5_224_86.10.pth.tar"))
    model.reset_classifier(116)
    # model = model.cuda()
    model.train()
    y1, y2, y3 = model(x)
    print(y1.shape)
    print(y2.shape)
    print(torch.sum(y2, dim=-1))
    print(torch.sum(y2, dim=-1).shape)
    print(y3)
    model.eval()
    y = model(x)
    print(y.shape)