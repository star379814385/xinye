...
class OutlookAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        out = (attn @ v).permute(0, 1, 4, 3, 2).reshape(B, C * self.kernel_size * self.kernel_size,
                                                        h * w)  # B,dimxkxk,H*W
        out = F.fold(out, output_size=(H, W), kernel_size=self.kernel_size, padding=self.padding,
                     stride=self.stride)  # B,C,H,W
...
if __name__ == '__main__':
    # input = torch.randn(2, 7, 7, 2048)
    input = torch.randn(2, 2048, 7, 7)
    outlook = OutlookAttention(dim=2048)
