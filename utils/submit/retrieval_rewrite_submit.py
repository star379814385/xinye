...
############### set data
roi_size = (224, 224)
json_file = json.load(open("./submit_942145.json", "r"))
print("待更改标注数量为{}".format(len(json_file["annotations"])))
query_data = QuiryDataset(root="./data/初赛数据集/test/a_images_crop", phase="val", width=roi_size[0], height=roi_size[1])
# 验证图像数量与json标注数量一致
assert len(query_data.samples) == len(json_file["annotations"]), "图像数量与待更改标注数不一致,分别为{},{}".format(
    len(query_data.samples), len(json_file["annotations"]))
