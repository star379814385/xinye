...
def split_json(save_root="./", train_p=0.8, is_shuffle=True):
    ...
    #　更改为一类检测
    train_annotations_info["categories"] = [{"id": 0, "name": "object"}]
    val_annotations_info["categories"] = train_annotations_info["categories"]
    for i in range(len(train_annotations_info["annotations"])):
        train_annotations_info["annotations"][i]["category_id"] = 0
    for i in range(len(val_annotations_info["annotations"])):
        val_annotations_info["annotations"][i]["category_id"] = 0
    ...
    # write
    with open(save_root + "train_object.json", "w") as f:
        json.dump(train_annotations_info, f)

    with open(save_root + "val_object.json", "w") as f:
