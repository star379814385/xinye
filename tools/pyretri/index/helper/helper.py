...
class IndexHelper:
    ...
    def save_topk_retrieved(self, single_query_info: Dict, topk: int, gallery_info: List[Dict], query_img: str) -> None:
        """
        Save the top-k retrieved images of one query.
        ...
        query_idx = single_query_info["ranked_neighbors_idx"]
        query_topk_idx = query_idx[:topk]
        with open('/data/retrie_ret/ret.txt','a') as f:
            for idx in query_topk_idx:
                img_name = os.path.basename(gallery_info[idx]["path"])
                f.write('{},{}'.format(query_img,img_name))
    ...
    def do_index(self, query_fea: np.ndarray, query_info: List, gallery_fea: np.ndarray) -> (List, np.ndarray, np.ndarray):
        ...
        """
        print('done0')
        for dim_proc in tqdm(self.dim_procs):
        ...
        print('done5')
        for i, info in tqdm(enumerate(query_info)):
            info["ranked_neighbors_idx"] = sorted_index[i].tolist()[:10]
